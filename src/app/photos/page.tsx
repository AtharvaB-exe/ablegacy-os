            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-[#1A1A1A] w-full max-w-md rounded-3xl p-8 relative shadow-2xl border border-white/10">
              ...
              {!isAuthenticated ? (
                <div className="text-center">
                  ...
                  <p className="text-gray-400 mb-6 text-sm">Enter security code.</p>
                  <form onSubmit={handleLogin} className="flex gap-2">
                    <input type="password" placeholder="Pass: admin" value={password} onChange={(e) => setPassword(e.target.value)} className="flex-1 bg-black/50 border border-white/20 rounded-xl px-4 py-3 outline-none focus:border-white text-white" />
